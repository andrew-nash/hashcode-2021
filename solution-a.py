...
adj_back = [sorted(x) for x in adj_back]
routes = []
for i in range(V):
...
try:
    ...
    uber_popular_streets = sorted(street_appearances)[-len(street_appearances)//200]
    tremendously_popular_streets = sorted(street_appearances)[-len(street_appearances)//500]
    ultra_popular_streets = sorted(street_appearances)[-len(street_appearances)//1000]
except:
...
for i in range(I):
    ...
    for j in range(B):
        current_street = street_appearances[street_uid[adj_back[i][j][2]]]
        if 0!= (current_street):
            #cnt = max(1,current_street//total_cars*10)
            #fc += 1
            if current_street > ultra_popular_streets:
                cnt=15
                fc+=1
            elif current_street > tremendously_popular_streets:
                cnt=10
                fc+=1
            elif current_street > uber_popular_streets:
                cnt=8
                fc+=1
            elif current_street > super_popular_streets:
                cnt=7
                fc+=1
